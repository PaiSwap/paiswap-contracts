...
contract PaiUni is Ownable {
    ...
    // The PAI TOKEN!
    PaiToken public pai;
    // Block number of distributing PAI period ends.
    uint256 public endBlock;
    // Block number of distributing bonus PAI period ends.
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256 _pai, uint256 _uni) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        _pai = user.amount.mul(accPaiPerShare).div(1e12).sub(user.paiRewardDebt);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pendingPai = user.amount.mul(pool.accPaiPerShare).div(1e12).sub(user.paiRewardDebt);
        uint256 pendingUni = user.amount.mul(pool.accUniPerShare).div(1e12).sub(user.uniRewardDebt);
        user.amount = user.amount.add(_amount);
        user.paiRewardDebt = user.amount.mul(pool.accPaiPerShare).div(1e12);
