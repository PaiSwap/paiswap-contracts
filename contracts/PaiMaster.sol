...
// PaiMaster is the master of Pai. He can make Pai and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PAI is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PaiMaster is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PAIs to distribute per block.
        uint256 lastRewardBlock; // Last block number that PAIs distribution occurs.
        uint256 accPaiPerShare; // Accumulated PAIs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus PAI period ends.
    uint256 public bonusEndBlock;
    // Block number when mint PAI period ends.
    uint256 public mintEndBlock;
    // PAI tokens created per block.
    uint256 public paiPerBlock;
    // Bonus muliplier for 5~20 days pai makers.
    uint256 public constant BONUSONE_MULTIPLIER = 20;
    // Bonus muliplier for 20~35 pai makers.
    ...
    constructor(
        PaiToken _pai,
        address _devaddr,
        uint256 _paiPerBlock,
        uint256 _startBlock
    ) public {
        pai = _pai;
        devaddr = _devaddr;
        paiPerBlock = _paiPerBlock;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        require(lpTokenPID[address(_lpToken)] == 0, "PaiMaster:duplicate add.");
    ...
    // Update the given pool's PAI allocation point. Can only be called by the owner.
    ...
    // Handover the paitoken mintage right.
    function handoverPaiMintage(address newOwner) public onlyOwner {
        pai.transferOwnership(newOwner);
    }
    ...
    // View function to see pending PAIs on frontend.
    function pendingPai(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPaiPerShare = pool.accPaiPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 paiReward = multiplier.mul(paiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accPaiPerShare = accPaiPerShare.add(paiReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accPaiPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 paiReward = multiplier.mul(paiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pai.mint(devaddr, paiReward.div(15));
        pai.mint(address(this), paiReward);
        pool.accPaiPerShare = pool.accPaiPerShare.add(paiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accPaiPerShare).div(1e12).sub(user.rewardDebt);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accPaiPerShare).div(1e12);
        if (pending > 0) safePaiTransfer(msg.sender, pending);
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from PaiMaster.
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accPaiPerShare).div(1e12);
        safePaiTransfer(msg.sender, pending);
    ...
    function safePaiTransfer(address _to, uint256 _amount) internal {
