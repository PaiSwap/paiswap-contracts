...
const PaiSwapBatchTrade = artifacts.require('PaiSwapBatchTrade');
const PaiSwapSlippageToken = artifacts.require('PaiSwapSlippageToken');
const { BN } = require('@openzeppelin/test-helpers');
const { web3 } = require('@openzeppelin/test-helpers/src/setup');

contract('PaiSwapBatchTrade', ([alice, bob, minter]) => {
    beforeEach(async () => {
        this.weth = await Weth.new();
        this.factory = await PaiSwapFactory.new(alice);
        this.router = await PaiSwapRouter.new(this.factory.address, this.weth.address)
        ...
        this.paiSwapBatchTrade = await PaiSwapBatchTrade.new(this.router.address, this.weth.address, this.factory.address);
        this.tokenABPair = await PaiSwapPair.at((await this.factory.createPair(this.tokenA.address, this.tokenB.address)).logs[0].args.pair);
        this.stokenAB = await PaiSwapSlippageToken.at((await this.tokenABPair.stoken()));
        await this.tokenA.approve(this.router.address, '1000000000000000000000');
        this.tokenAETHPair = await PaiSwapPair.at((await this.factory.createPair(this.tokenA.address, this.weth.address)).logs[0].args.pair);
        this.stokenAETH = await PaiSwapSlippageToken.at((await this.tokenAETHPair.stoken()));
    ...
    it('should swap from tokenA to tokenB 10 times successfully', async () => {
        ...
        await this.tokenA.approve(this.paiSwapBatchTrade.address, 10000, { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenA.address, this.tokenB.address, 10000, 10, false, { from:bob });
        ...
        assert.equal((await this.stokenAB.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
        assert.equal((await this.tokenA.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
        assert.equal((await this.tokenB.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
    ...
    it('should swap from tokenA to tokenB 10 times and add liquidity successfully', async () => {
        ...
        await this.tokenA.transfer(bob, 10000);
        await this.tokenA.approve(this.paiSwapBatchTrade.address, 10000, { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenA.address, this.tokenB.address, 10000, 10, true, { from:bob });
        console.log("gas used when swap token 10 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAB.balanceOf(bob);
        console.log("stoken amount when swap token 10 times:" + bal.toString());
    ...
    it('should add liquidity successfully', async () => {
        ...
        await this.tokenA.transfer(bob, 10000);
        await this.tokenA.approve(this.paiSwapBatchTrade.address, 10000, { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenA.address, this.tokenB.address, 10000, 0, true, { from:bob });
        console.log("gas used when swap token 10 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAB.balanceOf(bob);
        console.log("stoken amount when swap token 10 times:" + bal.toString());
        assert.equal((await this.stokenAB.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
    ...
    it('should swap from tokenA to tokenB 20 time successfully', async () => {
        ...
        await this.tokenA.transfer(bob, 10000);
        await this.tokenA.approve(this.paiSwapBatchTrade.address, 10000, { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenA.address, this.tokenB.address, 10000, 20, false, { from:bob });
    ...
    it('should swap from ETH to other tokens 10 times successfully', async () => {
        ...
        const tx = await this.paiSwapBatchTrade.swapExactETHForTokens(this.tokenA.address, 10, false, { value:'5000000000000000000' });
    ...
    it('should swap from ETH to other tokens and add liquidity successfully', async () => {
        ...
        const tx = await this.paiSwapBatchTrade.swapExactETHForTokens(this.tokenA.address, 10, true, { value:'1000000000000000000' });
        console.log("gas used when swap eth 10 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAETH.balanceOf(alice);
        console.log("stoken amount when swap eth 10 times:" + bal.toString())
        const ethAfter = await web3.eth.getBalance(alice);
        ...
        assert.equal((await this.stokenAETH.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
        assert.equal((await this.weth.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
        assert.equal((await this.tokenA.balanceOf(this.paiSwapBatchTrade.address)).valueOf(), '0');
        console.log(tx.logs[0].args.consume.toString(), tx.logs[0].args.stoken.toString(), tx.logs[0].args.lptoken.toString());
    });
    ...
    it('should add ETH liquidity successfully', async () => {
        const ethBefore = await web3.eth.getBalance(alice);
        const LPBefore = await this.tokenAETHPair.balanceOf(alice);
        const tx = await this.paiSwapBatchTrade.swapExactETHForTokens(this.tokenA.address, 0, true, { value:'1000000000000000000' });
        console.log("gas used when swap eth 10 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAETH.balanceOf(alice);
        console.log("stoken amount when swap eth 10 times:" + bal.toString())
        const ethAfter = await web3.eth.getBalance(alice);
    ...
    it('should swap from ETH to other tokens 20 times successfully', async () => {
        const tx = await this.paiSwapBatchTrade.swapExactETHForTokens(this.tokenA.address, 20, false, { value:'5000000000000000000' });
    ...
    it('simulate ETH/PAI', async () => {
        ...
        await this.tokenB.approve(this.paiSwapBatchTrade.address, '1000000000000000000', { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenB.address, this.tokenA.address, '1000000000000000000', 20, false, { from:bob });
        console.log("gas used when swap token 20 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAB.balanceOf(bob);
        console.log("stoken amount when swap token 20 times:" + bal.toString());
    ...
    it('simulate USDT/PAI', async () => {
        ...
        await this.tokenA.approve(this.paiSwapBatchTrade.address, '1000000000', { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenA.address, this.tokenB.address, '1000000000', 20, false, { from:bob });
        console.log("gas used when swap token 20 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAB.balanceOf(bob);
        console.log("stoken amount when swap token 20 times:" + bal.toString())
    ...
    it('simulate DAI/PAI', async () => {
        ...
        await this.tokenA.approve(this.paiSwapBatchTrade.address, '1000000000000000000000', { from:bob });
        const tx = await this.paiSwapBatchTrade.swapExactTokensForTokens(this.tokenA.address, this.tokenB.address, '1000000000000000000000', 20, false, { from:bob });
        console.log("gas used when swap token 20 times:" + tx.receipt.gasUsed);
        const bal = await this.stokenAB.balanceOf(bob);
        console.log("stoken amount when swap token 20 times:" + bal.toString())
