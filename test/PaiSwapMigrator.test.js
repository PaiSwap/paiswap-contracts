...
const PaiSwapMigrator = artifacts.require('PaiSwapMigrator');
const { keccak256, defaultAbiCoder, solidityPack } = require("ethers/utils");
const util = require("ethereumjs-util");

contract('PaiSwapMigrator', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.weth = await Weth.new();
        this.uniFactory = await PaiSwapFactory.new(alice);
        this.uniRouter = await PaiSwapRouter.new(this.uniFactory.address, this.weth.address)
        this.sushiFactory = await PaiSwapFactory.new(alice);
        this.sushiRouter = await PaiSwapRouter.new(this.sushiFactory.address, this.weth.address);
        this.paiFactory = await PaiSwapFactory.new(alice);
        this.paiRouter = await PaiSwapRouter.new(this.paiFactory.address, this.weth.address);
        this.token1 = await MockERC20.new('TOKEN1', 'TOKEN', '100000000');
        this.token2 = await MockERC20.new('TOKEN2', 'TOKEN2', '100000000');
        this.uniPair = await PaiSwapPair.at((await this.uniFactory.createPair(this.token1.address, this.token2.address)).logs[0].args.pair);
        this.sushiPair = await PaiSwapPair.at((await this.sushiFactory.createPair(this.token1.address, this.token2.address)).logs[0].args.pair);
        this.paiPair = await PaiSwapPair.at((await this.paiFactory.createPair(this.token1.address, this.token2.address)).logs[0].args.pair);
        this.paiSwapMigrator = await PaiSwapMigrator.new(this.uniFactory.address, this.uniRouter.address, this.sushiFactory.address,
            this.sushiRouter.address, this.paiRouter.address);
    ...
    it('should migrate from uniswap successfully', async () => {
        ...
        await this.token1.approve(this.paiRouter.address, '10000');
        await this.token2.approve(this.paiRouter.address, '10000');
        await this.paiRouter.addLiquidity(this.token1.address, this.token2.address, '10000', '10000', '0', '0', bar, '10000000000000');
        const paiLpAmount = await this.paiPair.balanceOf(bar);
        assert.equal(paiLpAmount.valueOf(), '9000');

        await this.uniPair.approve(this.paiSwapMigrator.address, '9000', { from: bar });
        await this.paiSwapMigrator.migrateUniswap(this.token1.address, this.token2.address, '9000', { from: bar });
        assert.equal((await this.uniPair.balanceOf(bar)).valueOf(), '0');
        assert.equal((await this.paiPair.balanceOf(bar)).valueOf(), '18000');
    });
    ...
    it('should migrate from sushiswap successfully', async () => {
        ...
        await this.sushiPair.approve(this.paiSwapMigrator.address, '9000', { from: bar });
        await this.paiSwapMigrator.migrateSushiSwap(this.token1.address, this.token2.address, '9000', { from: bar });
    ...
    it('should migrate from uniswap with permit successfully', async () => {
        ...
        await this.token1.approve(this.paiRouter.address, '10000');
        await this.token2.approve(this.paiRouter.address, '10000');
        await this.paiRouter.addLiquidity(this.token1.address, this.token2.address, '10000', '10000', '0', '0', this.bob.address, '10000000000000');
        const paiLpAmount = await this.paiPair.balanceOf(this.bob.address);
        assert.equal(paiLpAmount.valueOf(), '9000');
        ...
        const digest = getApprovalDigest(this, this.bob.address, this.paiSwapMigrator.address, 9000, 0, 10000000000000);
        const priKeyBuf = util.toBuffer(this.bob.privateKey);
        const { v, r, s } = util.ecsign(Buffer.from(digest.slice(2), "hex"), priKeyBuf);
        await this.paiSwapMigrator.migrateUniswapWithPermit(this.token1.address, this.token2.address,
            '9000', '10000000000000', v, r, s, { from: this.bob.address });
        assert.equal((await this.uniPair.balanceOf(this.bob.address)).valueOf(), '0');
        assert.equal((await this.paiPair.balanceOf(this.bob.address)).valueOf(), '18000');
    });
    ...
    it('should migrate from sushiswap with permit successfully', async () => {
        ...
        const priKeyBuf = util.toBuffer(this.bob.privateKey);
        const { v, r, s } = util.ecsign(Buffer.from(digest.slice(2), "hex"), priKeyBuf);
        await this.paiSwapMigrator.migrateSushiSwapWithPermit(this.token1.address, this.token2.address,
            '9000', '10000000000000', v, r, s, { from: this.bob.address });
