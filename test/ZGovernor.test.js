...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.pai.delegate(dev, { from: dev });
        this.chef = await PaiMaster.new(this.pai.address, dev, '100', '0', { from: alice });
        ...
        // Perform another deposit to make sure some PAIs are minted in that 1 block.
        await this.chef.deposit(0, '100', { from: minter });
        assert.equal((await this.pai.totalSupply()).valueOf(), '1066');
        assert.equal((await this.pai.balanceOf(minter)).valueOf(), '1000');
        assert.equal((await this.pai.balanceOf(dev)).valueOf(), '66');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), { from: alice });
        this.gov = await GovernorAlpha.new(this.timelock.address, this.pai.address, alice, { from: alice });
